    ...
    options = {
      {
        ...
        type = "switch",
        default = true,
        desc = "关闭 EventLogSDK 与 U8 GameBI 的采集入口",
      },
      {
        key = "block_http",
        label = "拦截上报请求",
        type = "switch",
        default = true,
        desc = "丢弃 /event /batch_event /beat 等上报请求",
      },
      {
        key = "pause_beat",
        label = "暂停 SDK 心跳",
  ...
  },
  {
    ...
    options = {
      ...
      },
      {
        ...
        step = 16384,
        format = "%d B",
        desc = "单条结果序列化后的上限，超出则降级为错误说明（截图受它约束）",
      },
      {
      ...
      },
      {
        ...
        type = "switch",
        default = false,
        desc = "开放 client.eval（可执行任意 Lua，仅在受控调试环境打开）",
      },
      {
        key = "verbose_log",
        label = "详细日志",
        type = "switch",
        default = false,
        desc = "把自动化桥的动作写入 persistentDataPath/plugin_automation_trace.txt",
      },
    },
  },
  {
    ...
    options = {
      ...
      },
      {
        ...
        step = 8,
        format = "%d KB",
        desc = "单批序列化后的字节上限（超出则少带几条，避免超长请求）",
      },
      {
        key = "msg_max_len",
        label = "单条截断",
      ...
      },
      {
        ...
        type = "switch",
        default = true,
        desc = "回传 Unity 提供的调用堆栈（定位报错点必需，占用更多流量）",
      },
      {
        key = "sink_file",
        label = "写设备日志文件",
        type = "switch",
        default = true,
        desc = "同时在设备 persistentDataPath 写 DTS-unity-*.log，便于 adb pull",
      },
      {
        key = "sink_level",
        label = "设备文件级别",
